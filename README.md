...
## 5. Screenshots
> 프로덕션(mywiki.pe.kr, 게스트 모드)에서 직접 캡처한 화면입니다. 아래 자리에 이미지를 추가해주세요 —
> `docs/screenshots/` 폴더를 만들고 파일을 넣은 뒤, 각 줄의 `(스크린샷 추가 예정)`을
> `![설명](docs/screenshots/파일명.png)`으로 바꾸면 됩니다.

| 화면 | 설명 |
|---|---|
| 메인 대시보드 | 반도체 도메인 관심사 방사형 다이어그램 + 배치 진행 상태(수집/정제·검증/요약/보고서 생성 시각) + 7일 수집·채택 추이 (스크린샷 추가 예정) |
| 일일 리포트 | 오늘자 리포트 카드 + Word/PDF/PPT 다운로드 버튼 + 히스토리 목록 (스크린샷 추가 예정) |
| 카테고리 현황 | 6개 카테고리 분류 요약, 분류별 비중·키워드 도넛 차트, 전일 대비 증가 폭 (스크린샷 추가 예정) |
| 위키 | 이슈/주제 페이지 본문, 연동 키워드, 근거 출처 목록, 사이드바 카테고리 트리 (스크린샷 추가 예정) |
| 에이전트 | 팀 공유 대화방 목록 + 위키 근거 기반 질의응답 (스크린샷 추가 예정) |
