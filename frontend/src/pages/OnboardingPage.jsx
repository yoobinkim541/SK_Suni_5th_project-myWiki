...
//
// 원래 시안 구성 그대로입니다.
//   진행 단계 바 → "관심사를 알려 주세요" → 관심 키워드(복수) / 직무(택1) / 연령대(택1)
...
export default function OnboardingPage({
  onComplete,
  // 아래 3개는 정적 목업 캡처용으로만 씁니다. 실제 앱에서는 넘기지 않습니다.
  initialKeywords = [],
  initialRole = null,
  initialAge = null,
}) {
  ...
  function finish(selectedKeywords) {
    onComplete?.({ keywords: selectedKeywords, role, age });
  }
            ...
            <div className="ob-chips">
              {AGE_OPTIONS.map((a) => (
                <button
                  type="button"
                  key={a}
                  className={`ob-chip${age === a ? ' sel' : ''}`}
                  onClick={() => setAge((prev) => (prev === a ? null : a))}
                >
                  {a}
                </button>
              ))}
