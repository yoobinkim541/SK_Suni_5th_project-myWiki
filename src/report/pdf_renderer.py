...
def _build_section_flowables(index: int, section: PdfSection, styles: StyleSheet1) -> list[object]:
    ...
    if section.evidences:
        flowables.append(Paragraph(_xml_text(K_SOURCES), styles["Subheading"]))
...
def _build_issue_flowables(section: PdfSection, styles: StyleSheet1) -> list[object]:
    ...
    if section.evidences:
        source_heading = Paragraph(_xml_text(K_SOURCES), styles["Subheading"])
        evidence_table = _build_evidence_table(section.evidences, styles)
        flowables.append(KeepTogether([source_heading, evidence_table]))
