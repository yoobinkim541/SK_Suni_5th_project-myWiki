...
def make_artifact(*, artifact_type: str = "pdf", object_key: str = "ws-1/reports/report-1/pdf/v1.pdf") -> SavedReportArtifact:
...
def test_save_pdf_report_artifact_uploads_and_persists_metadata() -> None:
    supabase = FakeSupabase()
    payload = b"%PDF-1.4\nmock-pdf"

    artifact = save_pdf_report_artifact(
        report_id="report-1",
        workspace_id="ws-001",
        report_version=3,
        report_date=date(2026, 8, 3),
    ...
    assert artifact.object_key == "ws-001/reports/report-1/pdf/v3.pdf"
    ...
    assert supabase.last_upload["path"] == "ws-001/reports/report-1/pdf/v3.pdf"
    assert supabase.last_upload["file"] == payload


def test_create_and_save_pdf_artifact_uses_renderer_when_bytes_missing() -> None:
    supabase = FakeSupabase()
    report = make_report()
    ...
    text = _extract_text(uploaded["file"])
    assert "\uc77c\uc77c \ubcf4\uace0\uc11c" in text
    assert "\uc0d8\ud50c \uc139\uc158 \uc81c\ubaa9" in text


def test_create_and_save_pdf_artifact_does_not_mutate_inputs() -> None:
    supabase = FakeSupabase()
    report = make_report()
    original = deepcopy(report.model_dump())

    create_and_save_pdf_artifact(report=report, supabase=supabase)

    assert report.model_dump() == original


def test_generate_daily_report_can_create_pdf_artifact(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, object] = {}
    config = ReportGenerationConfig(artifacts={"formats": [ArtifactType.PDF]})
    ...
    monkeypatch.setattr(
        "src.report.interface.create_and_save_pdf_artifact",
    ...
    monkeypatch.setattr("src.report.interface.create_and_save_pptx_artifact", lambda **kwargs: (_ for _ in ()).throw(RuntimeError("should not be called")))
