...
def make_artifact(*, artifact_type: str = "markdown", object_key: str = "ws-1/reports/report-1/markdown/v1.md") -> SavedReportArtifact:
...
def test_save_docx_report_artifact_uploads_and_persists_metadata() -> None:
    supabase = FakeSupabase()
    payload = b"PK\x03\x04word-bytes"

    artifact = save_docx_report_artifact(
        report_id="report-1",
        workspace_id="ws-001",
        report_version=3,
        report_date=date(2026, 8, 3),
    ...
    assert artifact.object_key == "ws-001/reports/report-1/docx/v3.docx"
    assert artifact.mime_type == DOCX_CONTENT_TYPE
    assert artifact.storage_bucket == DEFAULT_REPORT_ARTIFACT_BUCKET
    assert supabase.last_upload is not None
    assert supabase.last_upload["path"] == "ws-001/reports/report-1/docx/v3.docx"
    assert supabase.last_upload["file"] == payload


def test_create_and_save_docx_artifact_uses_renderer_when_bytes_missing() -> None:
    supabase = FakeSupabase()
    report = make_report()

    artifact = create_and_save_docx_artifact(report=report, supabase=supabase)

    assert artifact.object_key.endswith(".docx")
...
def test_create_and_save_docx_artifact_does_not_mutate_inputs() -> None:
    supabase = FakeSupabase()
    report = make_report()
    original = deepcopy(report.model_dump())

    create_and_save_docx_artifact(report=report, supabase=supabase)

    assert report.model_dump() == original


def test_generate_daily_report_can_create_docx_artifact(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, object] = {}
    config = ReportGenerationConfig(artifacts={"formats": [ArtifactType.DOCX]})
    ...
    monkeypatch.setattr(
        "src.report.interface.create_and_save_docx_artifact",
        lambda **kwargs: captured.update(kwargs) or make_artifact(artifact_type="docx", object_key="ws-1/reports/report-1/docx/v1.docx"),
    )
    ...
    assert result.artifact.artifact_type == ArtifactType.DOCX
    assert result.artifacts[0].artifact_type == ArtifactType.DOCX
    assert result.report.artifact_type == ArtifactType.DOCX
